...
class VCOCOeval(object):
    ...
    def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1'):
        with open(detections_file, 'rb') as f:
            loaded_dets = pickle.load(f)

            dets = defaultdict(list)
            for det in loaded_dets:
                dets[det['image_id']].append(det)


        tp = [[[] for r in range(2)] for a in range(self.num_actions)]
        tp2 = [[[] for r in range(2)] for a in range(self.num_actions)]


        fp = [[[] for r in range(2)] for a in range(self.num_actions)]
        fp2 = [[[] for r in range(2)] for a in range(self.num_actions)]


        sc = [[[] for r in range(2)] for a in range(self.num_actions)]
        sc2 = [[[] for r in range(2)] for a in range(self.num_actions)]
        ...
        for i in tqdm.tqdm(range(len(vcocodb))):
            ...
            # breakpoint()
            # valid_gt = gt_actions.sum(axis=0) != 0
            for aid in range(self.num_actions):
                # s = time.time()
                ...
                for rid in range(len(self.roles[aid])-1): # action per AP
                    ...
                    overlaps_all = get_overlap_batch(gt_boxes, agent_boxes)

                    overlaps_b = overlaps_all[:, idx]
                    jmax_b = overlaps_b.argmax(axis=0)
                    ovmax_b = overlaps_b.max(axis=0)
                    # if matched with an instance with no annotations
                    # continue

                    ignore_idx_mask = ~ignore[jmax_b]
                    ignore_idx = idx[ignore_idx_mask]

                    # overlaps_b = overlaps_b[ignore_idx]
                    jmax_b = jmax_b[ignore_idx_mask]
                    ovmax_b = ovmax_b[ignore_idx_mask]

                    no_gt_idx = np.all(gt_roles[jmax_b,:] == -1,axis=1)

                    sc1_mask = np.logical_and(np.logical_or(np.all(role_boxes[ignore_idx,:] == 0,axis=1),np.all(np.isnan(role_boxes[ignore_idx, :]),axis=1)),no_gt_idx)
                    sc1_mask2 = np.logical_and(~np.logical_or(np.all(role_boxes[ignore_idx,:] == 0,axis=1),np.all(np.isnan(role_boxes[ignore_idx, :]),axis=1)),no_gt_idx)

                    ov_role_sc1 = np.zeros(len(ignore_idx))
                    ov_role_sc2 = np.zeros(len(ignore_idx))

                    ov_role_sc1[sc1_mask] = 1.
                    ov_role_sc1[sc1_mask2] = 0.
                    ov_role_sc2[no_gt_idx] = 1.

                    tmp_value = get_overlap_by(gt_roles[jmax_b, :].reshape((-1, 4)),role_boxes[ignore_idx, :])[~no_gt_idx]
                    ov_role_sc1[~no_gt_idx] = tmp_value
                    ov_role_sc2[~no_gt_idx] = tmp_value

                    is_true_action_b = (gt_actions[jmax_b,aid] == 1)
                    sc[aid][rid].extend(agent_scores[ignore_idx])
                    sc2[aid][rid].extend(agent_scores[ignore_idx])


                    tp_mask_sc1 = np.logical_and(is_true_action_b,np.logical_and((ovmax_b>=ovr_thresh),(ov_role_sc1>=ovr_thresh)))
                    tp_mask2_sc1 = np.zeros_like(tp_mask_sc1)
                    jmax_b_sc1 = copy.deepcopy(jmax_b)
                    jmax_b_sc1[~tp_mask_sc1] = -1
                    v,m = np.unique(jmax_b_sc1, return_index=True)
                    tp_mask2_sc1[m[v!=-1]] = True
                    tp_mask_sc1 = np.logical_and(tp_mask_sc1,tp_mask2_sc1)

                    tp_mask_sc2 = np.logical_and(is_true_action_b, np.logical_and((ovmax_b >= ovr_thresh),
                                                                                  (ov_role_sc2 >= ovr_thresh)))
                    tp_mask2_sc2 = np.zeros_like(tp_mask_sc2)
                    jmax_b_sc2 = copy.deepcopy(jmax_b)
                    jmax_b_sc2[~tp_mask_sc2] = -1
                    v, m = np.unique(jmax_b_sc2, return_index=True)
                    tp_mask2_sc2[m[v != -1]] = True
                    tp_mask_sc2 = np.logical_and(tp_mask_sc2, tp_mask2_sc2)

                    tp[aid][rid].extend(tp_mask_sc1.astype('int'))
                    fp[aid][rid].extend((~tp_mask_sc1).astype('int'))

                    tp2[aid][rid].extend(tp_mask_sc2.astype('int'))
                    fp2[aid][rid].extend((~tp_mask_sc2).astype('int'))

                    # for j in idx:
                    #     pred_box = agent_boxes[j, :]
                    #     # overlaps = get_overlap(gt_boxes, pred_box)
                    #     overlaps = overlaps_all[:,j]
                    #     # breakpoint()
                    ...
                    #         ov_role2 = get_overlap(gt_roles[jmax, :].reshape((1, 4)), role_boxes[j, :])
                    #     is_true_action = (gt_actions[jmax, aid] == 1)
                    #     sc[aid][rid].append(agent_scores[j])
                    #     sc2[aid][rid].append(agent_scores[j])
                    #
                    #     #scenario_1
                    ...
                    #
                    #     #scenario_2
                    #     if is_true_action and (ovmax>=ovr_thresh) and (ov_role2>=ovr_thresh):
                    #         if covered2[jmax]:
                    #             fp2[aid][rid].append(1)
                    #             tp2[aid][rid].append(0)
                    #         else:
                    #             fp2[aid][rid].append(0)
                    #             tp2[aid][rid].append(1)
                    #             covered2[jmax] = True
                    #     else:
                    #         fp2[aid][rid].append(1)
                    #         tp2[aid][rid].append(0)
                    # print()
                # print('3',time.time()-s)
        self.compute_ap(fp,tp,sc,npos,'scenario_1')
        self.compute_ap(fp2,tp2,sc2,npos,'scenario_2')
    ...
    def compute_ap(self,fp,tp,sc,npos,eval_type):
        # compute ap for each action
        role_ap = np.zeros((self.num_actions, 2), dtype=np.float32)
        role_ap[:] = np.nan
        for aid in range(self.num_actions):
...
def get_overlap_by(boxes, ref_box):
    ixmin = np.maximum(boxes[:, 0], ref_box[:,0])
    iymin = np.maximum(boxes[:, 1], ref_box[:,1])
    ixmax = np.minimum(boxes[:, 2], ref_box[:,2])
    iymax = np.minimum(boxes[:, 3], ref_box[:,3])
...
def get_overlap_batch(boxes, ref_box):
    ixmin = np.maximum(boxes[:, 0 ,np.newaxis], ref_box[:,0])
    iymin = np.maximum(boxes[:, 1,np.newaxis], ref_box[:,1])
    ixmax = np.minimum(boxes[:, 2,np.newaxis], ref_box[:,2])
    iymax = np.minimum(boxes[:, 3,np.newaxis], ref_box[:,3])
    iw = np.maximum(ixmax - ixmin + 1., 0.)
    ih = np.maximum(iymax - iymin + 1., 0.)
    inters = iw * ih
    ...
    # union
    uni = ((ref_box[:,2] - ref_box[:,0] + 1.) * (ref_box[:,3] - ref_box[:,1] + 1.) +
           (boxes[:, 2,np.newaxis] - boxes[:, 0,np.newaxis] + 1.) *
           (boxes[:, 3,np.newaxis] - boxes[:, 1,np.newaxis] + 1.) - inters)
